...
module.exports = {
	...
	compareAssociationRecords: function(
		association,
		sourceRecords,
		targetRecords,
		newRecordCallback,
			...
			.forEach(function(sourceRecord) {
				...
				var targetRecord = targetRecords
					.find(function(targetRecord, index) {
						...
						if(sourceRecord[association.sourcePivotKey] === targetRecord[association.targetPivotKey]) {
				...
				if(!targetRecord) {
					newRecordCallback(sourceRecord);
					return;
				}
				...
				QueryHelper.checkRecordForUpdates(
